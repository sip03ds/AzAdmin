...
$user = Get-ADUser -Identity pte
Add-Content $ExecutionLogFile $user
$PasswordPol = (Get-ADUserResultantPasswordPolicy -Identity $user.Name)
Add-Content $ExecutionLogFile "CHECK"
Add-Content $ExecutionLogFile $PasswordPol
Add-Content $ExecutionLogFile "CHECK COMPLETE"
...
foreach ($user in $users)
{
	...
	if (($samAccountName) -eq "pte") 
	{
	Add-Content $ExecutionLogFile $user
	Add-Content $ExecutionLogFile $Name
	Add-Content $ExecutionLogFile $emailaddress
	Add-Content $ExecutionLogFile $passwordSetDate
	Add-Content $ExecutionLogFile $samAccountName
	Add-Content $ExecutionLogFile $pwdLastSet
	Add-Content $ExecutionLogFile $PasswordPol
	Add-Content $ExecutionLogFile $maxPasswordAge
	Add-Content $ExecutionLogFile $daysToExpire
	Add-Content $ExecutionLogFile "PASOK"
	  if (($PasswordPol) -ne $null) {
		Add-Content $ExecutionLogFile $PasswordPol
	  }
	  else {
		Add-Content $ExecutionLogFile "THELW NA PSIFIZW PASOK"
	  }
	Add-Content $ExecutionLogFile $PasswordPol
	}
...
foreach ($user in $notifyUsers)
{
	...
	<td style=""width: 613px;vertical-align: middle;""><img src=""https://stweuinfpsta001.blob.core.windows.net/endpoint/logos/email.jpg?sv=2019-02-02&st=2020-08-03T08%3A45%3A13Z&se=2020-08-04T08%3A45%3A13Z&sr=b&sp=r&sig=lV3gfmNgLT4hBnAGNPJm7Ur6iMiqeAPe1k5rTfSIte4%3D"" alt=""Viohalco Logo"" width=""256"" height=""51"" /></td>
	</tr>
	<tr>
