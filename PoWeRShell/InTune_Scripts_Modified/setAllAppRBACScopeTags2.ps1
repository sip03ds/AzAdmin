...
$sctg = Get-RBACScopeTag
Write-Host "SCOPES: "$sctg
Write-Host "IDs "$sctg.id

$apps =  Get-IntuneApplication 
foreach ($app in $apps) {
    $appIdentifier = $app.id
    $appInfo = Get-IntuneApplication -id $appIdentifier
    $appid = $appInfo.id;
    $appdt = $appInfo.'@odata.type';
    Write-Host "App ID: $($appInfo.id)"
    $Result = Update-IntuneApplication -id $appid -Type $appdt -ScopeTags $sctg.id
}
